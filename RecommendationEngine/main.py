...
HACK_DATA = "https://raw.githubusercontent.com/arjun619/recommendation_ecommerce/main/hack.csv"

@app.route('/')
@cross_origin(origin='*')
def hello():
    if 'q' not in request.args:
        return "Pls give arg ?q=userid"
    q = request.args['q']
    q=int(q)
    # return q
    path = os.path.join(__location__, 'hack.csv')
    data=pd.read_csv(HACK_DATA)
    # print(data[data["product_id"]==463570586]["brand"])
    path = os.path.join(__location__, 'content_utility_data');
    group= pd.read_csv(path)
    print("group data found")
    path = os.path.join(__location__, 'newcontentresult')
    ...
    temp=pd.read_csv("https://raw.githubusercontent.com/arjun619/recommendation_ecommerce/main/collaborative_newdataset.csv")
...
def samebrand():
    if 'q' not in request.args:
        return "Pls give arg /samebtand/?q=productid"
    q = request.args['q']
    q=int(q)
    print(type(q))
    data= pd.read_csv(HACK_DATA)
...
def sametype():
    if 'q' not in request.args:
        return "Pls give arg /sametype/?q=productid"
    q = request.args['q']
    q=int(q)
    print(type(q))
    data= pd.read_csv(HACK_DATA)
